...
class Trainer():  # merge
    ...
    def train(self):
        ...
        bar = Bar(f'Epoch {self.epoch + 1}/{self.end_epoch}', fill='*', max=training_iter * len(train_dataloader))
        ...
        for data_index in range(len(train_dataloader)):
            present_train_dataloader = train_dataloader[data_index]
            for iter_index in range(Iter_sum, Iter_sum + len(present_train_dataloader)):
        # ****************************************************************************
                ...
                loss_total, loss_accel = self.loss(denoised_pos, data_gt)
                ...
                summary_string = f'(Iter {iter_index + 1} | Total: {bar.elapsed_td} | 'f'ETA: {bar.eta_td:} | loss: {loss_total:.4f} | Accel loss : {loss_accel:.3f}'

                self.writer.add_scalar('train_loss', loss_total, global_step=self.train_global_step)            # train_loss
                self.writer.add_scalar('accel_loss', loss_accel, global_step=self.train_global_step)            # accel_loss
            ...
            Iter_sum += len(present_train_dataloader)
    ...
    def evaluate_2d(self, dataset_index, dataset):
        ...
        eval_dict = evaluate_smoothnet_2D(self.model, self.test_dataloader[dataset_index],
                                          self.device, self.cfg, dataset=dataset, dataset_index=dataset_index)

        log_str = f'Epoch {self.epoch + 1}, '

        print(self.cfg.DATASET_NAME)

        if self.cfg.DATASET_NAME == "jhmdb":
            log_str += "" + ' '.join(
                [f'{k.upper()}: {v * 100:.2f}%,' for k, v in eval_dict.items()])
        elif self.cfg.DATASET_NAME == "h36m" \
                or self.cfg.DATASET_NAME == "h36m_strength" \
                or self.cfg.DATASET_NAME == "h36m_static"\
                or self.cfg.DATASET_NAME == "h36m_combine":
