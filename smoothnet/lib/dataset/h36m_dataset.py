from smoothnet.lib.dataset import BaseDataset
import numpy as np
import os
import bisect
...
class H36MDataset(BaseDataset):
    ...
    def __init__(self, cfg, estimator='fcn', return_type='3D', phase='train'):
        ...
        if return_type in ['3D', 'smpl', '2D', '2D_hrnet']:
        ...
        ground_truth_data_len = sum(len(seq) for seq in ground_truth_data["imgname"])
        # 序列求和
        detected_data_len = sum(len(seq) for seq in detected_data["imgname"])
        if ground_truth_data_len != detected_data_len:
            raise ImportError("Detected data is not the same size with ground_truth data!")
        ...
        if self.return_type == '3D':
            ...
            if cfg.TRAIN.USE_6D_SMPL:
                ...
                for i in range(len(self.ground_truth_data_pose)):
                    self.ground_truth_data_pose[i] = numpy_axis_to_rot6D(self.ground_truth_data_pose[i].reshape(-1, 3)).reshape(-1, self.input_dimension)

                for i in range(len(self.detected_data_pose)):
                    self.detected_data_pose[i] = numpy_axis_to_rot6D(self.detected_data_pose[i].reshape(-1, 3)).reshape(-1, self.input_dimension)
            else:
    ...
    def get_data(self, index):
        ...
        test_data_self = self.ground_truth_data_joints_2d[position]
        ...
        if self.return_type == '3D':
            ...
            gt_data = (self.ground_truth_data_joints_2d[position] - H36M_IMG_SHAPE / 2) / (H36M_IMG_SHAPE / 2)
            pred_data = (self.detected_data_joints_2d[position] - H36M_IMG_SHAPE / 2) / (H36M_IMG_SHAPE / 2)

        elif self.return_type == 'smpl':
            gt_data = self.ground_truth_data_pose[position].reshape(ground_truth_data_len, -1)
            pred_data = self.detected_data_pose[position].reshape(ground_truth_data_len, -1)

        if self.slide_window_size <= ground_truth_data_len:
            gt_data = np.concatenate((gt_data, np.zeros(tuple((1,)) + tuple(gt_data.shape[1:]))), axis=0)
            pred_data = np.concatenate((pred_data, np.zeros(tuple((1,)) + tuple(pred_data.shape[1:]))), axis=0)

            start_idx = (index - self.data_start_num[position]) % (ground_truth_data_len - self.slide_window_size + 1)
            ...
            gt_data = np.concatenate((gt_data, np.zeros(tuple((self.slide_window_size - ground_truth_data_len,)) + tuple(gt_data.shape[1:]))), axis=0)
            pred_data = np.concatenate((pred_data, np.zeros(tuple((self.slide_window_size - ground_truth_data_len,)) + tuple(pred_data.shape[1:]))), axis=0)
    ...
    def get_test_data(self, index):
        ...
        if self.return_type == '3D':
            ...
            gt_data = self.ground_truth_data_pose[index].reshape(ground_truth_data_len, -1)
            pred_data = self.detected_data_pose[index].reshape(ground_truth_data_len, -1)

            gt_shape = self.ground_truth_data_shape[index].reshape(ground_truth_data_len, -1)
            pred_shape = self.detected_data_shape[index].reshape(ground_truth_data_len, -1)
