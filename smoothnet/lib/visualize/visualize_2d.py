import os
from smoothnet.lib.utils.eval_metrics import calculate_mpjpe, calculate_pampjpe, calculate_accel_error
import cv2
import numpy as np
import torch
import matplotlib
...
from smoothnet.lib.visualize.visualize_skeleton import *
...
def plot_skeleton(ax_2d, skeleton, color, edges, joints):
    ...
    for idx, lr in joints:
        ...
        ax_2d.plot(skeleton[idx, 0], skeleton[idx, 1], joint_color)
        ax_2d.text(skeleton[idx, 0], skeleton[idx, 1], str(idx))

    for kpta, kptb, lr in edges:
        line_color = color[0] if lr else color[1]
        ax_2d.plot(skeleton[[kpta, kptb], 0], skeleton[[kpta, kptb], 1], line_color)
...
def visualize_2d(vis_output_video_path,
    ...
    # 将visualize_skeleton.py中 'H36M_HRNET_2D_EDGES' 对应的数据读出来
    """
    HRNet-H36M_HRNET_2D_EDGES: 0 是 red, 1 是 black
    [[9, 8, 0],
     [8, 7, 0],
     [7, 0, 0],
     [4, 0, 0],
    ...
    # 将visualize_skeleton.py中 'H36M_HRNET_2D_JOINTS' 对应的数据读出来
    """
    HRNet-H36M_HRNET_2D_JOINTS: 0 是 red, 1 是 black
    [[0, 0],
     [1, 1],
     [2, 1],
     [3, 1],
    ...
    acc_in = np.concatenate((np.array([0]), np.array(calculate_accel_error(data_pred.reshape(len_seq, -1, 2), data_gt.reshape(len_seq, -1, 2)).cpu()), np.array([0])))
    acc_out = np.concatenate((np.array([0]), np.array(calculate_accel_error(predicted_pos.reshape(len_seq, -1, 2), data_gt.reshape(len_seq, -1, 2)).cpu()), np.array([0])))
    ...
    # Video Writer Config
    videoWriter = cv2.VideoWriter(os.path.join(vis_output_video_path, vis_output_video_name), cv2.VideoWriter_fourcc(*'mp4v'), FRAME_RATE,
                                  (SUB_FIG_SIZE * VIEW_NUM * len(anim_output) * SUB_FIG_UNIT, SUB_FIG_SIZE * 3 * SUB_FIG_UNIT))

    # max(0, start_frame) = 0
    # min(len_seq, end_frame) = 1000

    # trange() -> return tqdm(_range(*args), **kwargs)
    for frame_i in trange(max(0, start_frame), min(len_seq, end_frame)):
        ...
        for view_i in range(VIEW_NUM):
            for index, (title, data) in enumerate(anim_output.items()):
                ...
                ax_2d = fig.add_subplot(3, len(anim_output) * VIEW_NUM, (view_i) * len(anim_output) + index + 1)
                ...
                plot_skeleton(ax_2d, data[frame_i, :, :], SKELRTON_COLOR, dataset_edges, dataset_joints)  # 绘制关节点
                # SKELRTON_COLOR = ['red', 'black']
