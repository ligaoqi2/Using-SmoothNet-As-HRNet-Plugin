...
class SmoothNet(nn.Module):
    ...
    def __init__(self,
        ...
        self.encoder = nn.Sequential(
            nn.Linear(window_size, hidden_size),                # 32 -> 512
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        assert T == self.window_size, ('Input sequence length must be equal to the window size. ', f'Got x.shape[2]=={T} and window_size=={self.window_size}')
...
if __name__ == "__main__":
    print(f"Test SmoothNet Module Start...")

    model = SmoothNet(window_size=32, output_size=32, hidden_size=512, res_hidden_size=128, num_blocks=5, dropout=0.25)
    inputs = torch.randn([1024, 2, 32]).cuda()
    summary(model, input_size=inputs.shape)
    output = model(inputs)
    print(f"Test SmoothNet Module End...")
